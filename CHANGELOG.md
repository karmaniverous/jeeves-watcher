...
#### [0.5.0](https://github.com/karmaniverous/jeeves-watcher/compare/0.4.4...0.5.0)

> 24 February 2026

**Inference Rules v2 — Schema-Based Metadata with Type Coercion**

This release implements a complete redesign of the inference rules system, replacing the `set` property with declarative JSON Schemas and adding type coercion, UI hints, runtime values tracking, and comprehensive config introspection APIs.

**Breaking Changes**

- `set` property removed from inference rules - replaced by `schema` arrays referencing named global schemas
- Inference rules now require `name` (string, unique) and `description` (string) fields
- `mapHelpers` and `templateHelpers` changed from `{ paths: string[] }` to `Record<name, { path, description? }>` (namespaced helpers)
- `GET /status` no longer returns `payloadFields` - use `POST /config/query` for schema introspection
- Config files using `set` syntax must migrate to `schema` format (no backward compatibility)

**Added**

- **Global schemas collection** - Named schema definitions in top-level `schemas` config, referenced by rules
- **Schema arrays on rules** - `schema` property accepts array of named refs and inline objects, merged left-to-right
- **Type coercion** - String → integer/number/boolean/array/object based on declared `type`
- **`uiHint` keyword** - UI rendering hints (`text`, `number`, `date`, `select`, `multiselect`, `check`)
- **`set` keyword** - Interpolation templates now live within property schemas, coerced to declared type
- **`matched_rules` payload** - Every point includes array of rule names that matched the file
- **Values index** - Runtime-accumulated distinct values per property per rule (trackable primitives only)
- **Issues file** - Persistent `issues.json` tracking type collisions and interpolation errors
- **Schema completeness validation** - All properties must have declared `type` (no silent string-defaulting)
- **Config watch reindex control** - New `configWatch.reindex` setting (`issues` | `full` | `none`)
- **Top-level `description`** - Organizational context for LLM consumers
- **`search.scoreThresholds`** - Score interpretation boundaries (`strong`, `relevant`, `noise`)
- **Template helpers introspection** - Same approach as map helpers (JSDoc + exports discovery)
- **New API endpoints:**
  - `GET /config/schema` - JSON Schema of merged virtual document
  - `POST /config/query` - JSONPath queries with optional `resolve: ["files", "globals"]`
  - `POST /config/match` - Path testing against rules + watch scope
  - `GET /issues` - Current embedding failures with error details
  - `POST /metadata` - Now validates against resolved schema for matched rules

**Changed**

- Helper sources now namespaced - `mapHelpers` and `templateHelpers` use `Record<name, { path, description? }>` format
- Helpers registered with namespace prefix (e.g., `slack_extractParticipants`, `{{date_toUnix}}`)
- Helper descriptions derived from `@module` / `@packageDocumentation` JSDoc tags (config `description` overrides)
- Config query endpoint operates over merged virtual document (authored config + runtime values + issues)
- Issues reindex (`configWatch.reindex: "issues"`) only re-processes files in issues file (targeted, cheap)
- Full reindex clears entire values index and rebuilds from scratch

**Fixed**

- Empty string interpolation results now properly coerce to `undefined` for non-string types
- Schema merge now correctly preserves property-level keywords across multiple refs
- Type collision detection prevents conflicting metadata from reaching Qdrant

**Documentation**

- Rewrote `guides/inference-rules.md` - complete coverage of schemas, type coercion, `uiHint`, merge semantics, `matched_rules`
- Added PlantUML diagrams:
  - Processing pipeline (file → match → schema merge → type coercion → embed)
  - Schema merge flow (global schemas + inline → merged schema → set extraction)
  - Config watch reindex flow
- Updated all API endpoint docs with new signatures and response shapes
- Added config reference for `schemas`, `search.scoreThresholds`, `configWatch.reindex`, `description`
- Updated getting-started examples to use schema format
- Added engineering standards section to spec
